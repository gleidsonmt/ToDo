...
// default -> {
// drawerItem.numberOfNotificationsProperty()
// .bind(Bindings.size(view.getList().getItems().filtered(e ->
// !e.isCompleted())));
// }
...
// // Default lists
// // List daily = new List(I18n.get("drawer.list.myDay"), ListType.DAILY,
// // Icon.SUN, true);
// // List tasks = new List(I18n.get("drawer.list.task"), ListType.TASKS,
// // Icon.HOME, true);
// // List important = new List(I18n.get("drawer.list.important"),
// // ListType.IMPORTANT, Icon.STAR, true);
...
// // important.setItems(data.filtered(task -> task.isImportant() &&
// // !task.isCompleted()));

// // List tasksCompleted = new List(I18n.get("drawer.list.task"),
// // ListType.TASKS, Icon.HOME);
// // List tasksIncompleted = new List(I18n.get("drawer.list.task"),
// // ListType.TASKS, Icon.HOME);
// // tasksIncompleted.setItems(data.filtered(task -> task.getListId() == 0
// // && !task.isCompleted()));
// // tasksCompleted.setItems(data.filtered(task -> task.isCompleted() &&
// // task.getListId() == 0));
...
// // // adding fixed views
// // getItems().addAll(new ViewList(daily), new ViewList(tasks), new
// // ViewList(important), new ViewList(completed),
// // new ViewList(all));
...
// // List all = getItems().stream().filter(list -> (list instanceof
// // ViewList)).map(list -> (ViewList) list)
// // .filter(viewList ->
// //
// viewList.getList().getType().equals(ListType.ALL)).findFirst().get().getList();

// // List completed = getItems().stream().filter(list -> (list instanceof
// // ViewList)).map(list -> (ViewList) list)
// // .filter(viewList ->
// // viewList.getList().getType().equals(ListType.COMPLETED)).findFirst().get()
// // .getList();
...
// // list.setItems(data.filtered(task -> task.getListId() ==
// // list.getId()));

// // List listCompleted = new List(list.getName());

// // listCompleted.setItems(
// // data.filtered(task -> task.getListId() == list.getId() &&
// // task.isCompleted()));
...
// // List listCompleted = new List(list.getName());
// // listCompleted.setItems(data.filtered(task -> task.getListId() ==
// list.getId() && task.isCompleted()));

// // List listIncompleted = new List(list.getName());
// // listIncompleted
// // .setItems(data.filtered(task -> task.getListId() == list.getId() &&
// !task.isCompleted()));
